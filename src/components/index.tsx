...
export function View({ data }: { data: BlogCard[] }) {
  useWaterMark();
...
export const ViewWithUtils = ({ data }: { data: BlogCard[] }) => {
  const [search, setSearch] = React.useState("");
  const [sort, setSort] = React.useState("");
  const filteredData = React.useMemo(() => {
    const result = search === "" ? [...data] : data.filter(post => {
    ...
    if (sort === "asc") return result.sort((a, b) => a.title.localeCompare(b.title));
    if (sort === "dsc") return result.sort((a, b) => b.title.localeCompare(a.title));
    return result;
  }, [data, search, sort]);
